  ...
  local _base_0 = {
    ...
    create_luasec_opts = function(self)
      ...
      if self.sock_type == "nginx" and key and cert then
        key = assert(ngx_ssl.parse_pem_priv_key(pl_file.read(key, true)))
        cert = assert(ngx_ssl.parse_pem_cert(pl_file.read(cert, true)))
      end
      return {
        key = key,
        certificate = cert,
    ...
    send_ssl_message = function(self)
      ...
      if t == MSG_TYPE.status then
        local _exp_0 = self.sock_type
        if "nginx" == _exp_0 then
          local luasec_opts = self:create_luasec_opts()
